...
void Renderer::makeResources()
{
    ...
    m_vertexBuffer = m_device->newBuffer(positions.data(), sizeof(simd_float2) * positions.size(), MTL::StorageModeShared);
...
void AppDelegate::applicationDidFinishLaunching(NS::Notification* notification)
{
    ...
    m_mtkView->setClearColor(MTL::ClearColor::Make(.95,.95,.95, 1));
s
