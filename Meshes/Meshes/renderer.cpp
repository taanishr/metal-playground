...
Renderer::Renderer(MTL::Device* device, MTK::View* view):
    ...
    m_frameSemaphore{1},
    m_time{0},
    m_mesh{5,150,simd_float4{0.0,0.5,0.8,1.0}, device}
{
...
void Renderer::makePipeline()
{
    ...
    renderPipelineDescriptor->setVertexDescriptor(m_mesh.m_vertexDescriptor);
...
void Renderer::draw()
{
    ...
    for (int i = 0; i < m_mesh.m_vertexBuffers.size(); ++i) {
        encoder->setVertexBuffer(m_mesh.m_vertexBuffers[i], 0, i);
    }
    
    encoder->drawIndexedPrimitives(m_mesh.m_primitiveType,
                                   m_mesh.m_indexCount,
                                   m_mesh.m_indexType,
